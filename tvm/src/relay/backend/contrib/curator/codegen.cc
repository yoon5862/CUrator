...
class CodegenResultNode : public Object {
 public:
  ...
  static constexpr const char* _type_key = "contrib.curator.CodegenResult";
...
TVM_REGISTER_GLOBAL("contrib.curator.CodegenResult")
...
GenerateBodyOutput GenerateBody(const std::string& func_name, const std::string& ext_func_id,
  ...
  const auto* instantiate_template_func =
      runtime::Registry::Get("contrib.curator.instantiate_template");
...
Target GetCutlassTarget() {
  ...
  if (!target.defined() || target->kind->name != "curator") {
...
class CodegenCutlass : public backend::MemoizedExprTranslator<std::vector<Output>>,
 ...
 private:
  ...
  GenerateBodyOutput GenerateBody(const CallNode* call, const std::string& func_name,
    ...
    return curator::GenerateBody(func_name, ext_func_id_, out_types_str, func_args, attrs,
                                 &buf_idx_);
  }
 ...
 private:
  ...
  static const FunctionNode* GetCutlassFunctionNode(const Expr& expr) {
    if (const auto* function_node = expr.as<FunctionNode>()) {
      ...
      if (opt_compiler.defined() && opt_compiler.value() == "curator") {
...
tvm::transform::Pass CompileForCutlassImpl() {
  auto pass_func = [=](IRModule mod, const tvm::transform::PassContext& pass_ctx) {
    ...
    const auto* pf = runtime::Registry::Get("relay.ext.curator.compile_for_cutlass");
...
TVM_REGISTER_GLOBAL("relay.ext.curator.create_c_source_module").set_body_typed(CreateCSourceModule);

tvm::transform::Pass CompileForCutlass() {
  return transform::Sequential(
      {transform::OutlineCompilerFunctionsWithExistingGlobalSymbols("curator"),
       CompileForCutlassImpl(), transform::MarkCompilerFunctionsAsExtern("curator")});
}
