...
def instantiate_gemm_template(attrs, func_args):
  ...
  cutlass::Status status = gemm_op.initialize(arguments${split_k_buffer_params}); //workspace.get() ${split_k_buffer_params}
  // CHECK(status == cutlass::Status::kSuccess);
  status = gemm_op();
  // CHECK(status == cutlass::Status::kSuccess);
"""
    ...
    has_split_k = int(attrs["split_k"]) > 1
    ...
    if has_split_k == True:
        aux_map.update(
            {
                "split_k_buffer": "void* workspace = (void*)(${arg3}->data);\n" if has_bias else "void* workspace = (void*)(${arg2}->data);\n",
                "split_k_buffer_params": ", workspace"
            }
        )
    else:
        aux_map.update(
